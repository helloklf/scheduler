...
8 个值**按位置对应 cpu0~cpu7**，每个位置是该 CPU 的"强制 PD"，非 0 即生效
（`cpuqos_v3_sync_task()`，L304-320，优先于一切分类结果）：

| 值 | 含义 |
|---|---|
| `0` | **不强制**（默认）。该核任务走正常分类（cgroup 组 / 任务级设置） |
| 非 0 | **强制 PD 编号**：任务被调度到该核运行时，L3 分区标签被强制写成该 PD，覆盖 cgroup 组设置与任务级设置 |
...
|---|---|
| `2` | CT（关键任务，吃 L3 优待） |
| `3` | 默认 NCT——效果接近"不强制"，但会**压住**别人单独给任务设的 CT |
| `10`~`15` | user group 1~6（PD = 9 + 组号），用哪几路 way 由该组掩码决定（方案一/二） |
| `1`, `4`~`9` | 固件侧预定义分区。内核侧唯一线索：`show_l3m_status` 把 PD2/PD6/PD7/PD8/PD9 都当 CT 类显示（L944-948），具体差异在闭源映射里 |
| `0` | 被占用为"关闭强制"的开关，**PD0 无法通过 custpd 强制** |

ioctl 预设（`CPUQOS_V3_SET_CUSTPD` mode=1，`set_custpd()` L1169-1191）写的就是
`14,14,14,14,2,2,2,2`，读法：**cpu0~3（小核）上的一切任务强制走 user group 5 的 way 掩码；
cpu4~7（中大核）上的一切任务强制 CT**——大核全优待，小核圈进可单独配额的组。

#### 两个易混点

1. **custpd（每核强制 PD）与 custpd_ctl（stall 闭环）是两套独立开关**：
   写 `custpd` 立即生效，不需要 `custpd_ctl=1`；闭环只周期改 group 1~3 的 way 掩码，
   不会改 custpd 数组。但若某核被强制到 PD10~15（user group），闭环改了该组掩码，
   效果会跟着变（PD10~15 的语义就是"用某组掩码"）。
2. **custpd 是"按核"不是"按任务"**：任务迁移到别的核，强制值就换成那个核的配置。
   想钉住单个线程请用 `set_ct_task`（TASK_RANK 跟任务走）；注意 custpd 的检查在最前面，
   会连任务级设置一起覆盖。
